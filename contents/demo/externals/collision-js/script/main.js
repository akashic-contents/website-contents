...
function circleToCircleDemo(scene) {
    const root = new g.E({ scene });
    const red = scene.asset.getImage("/image/red.png").asSurface();
    const green = scene.asset.getImage("/image/green.png").asSurface();
    const blue = scene.asset.getImage("/image/blue.png").asSurface();
    const c1 = {
...
function main(_param) {
    ...
    const scene = new g.Scene({
        game: g.game,
        assetPaths: ["/image/red.png", "/image/green.png", "/image/blue.png"]
    });
    let currentDemoE;
    scene.onLoad.add(() => {
