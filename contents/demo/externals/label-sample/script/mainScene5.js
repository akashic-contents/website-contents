...
function mainScene5() {
    ...
    const scene = new g.Scene({
        game: game,
        assetPaths: ["/image/bmpfont.png", "/text/bmpfont-glyph.json", "/image/mplus.png", "/text/mplus-glyph.json"]
    });
    const rate = game.fps / 2;
    scene.onLoad.add(() => {
        // グリフデータの生成
        const mPlusGlyphInfo = scene.asset.getJSONContent("/text/mplus-glyph.json");
        ...
        const bmpGlyphInfo = scene.asset.getJSONContent("/text/bmpfont-glyph.json");
        const bmpfont = new g.BitmapFont({
            src: scene.asset.getImage("/image/bmpfont.png"),
