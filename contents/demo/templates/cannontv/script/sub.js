...
function createGameBg(scene) {
	...
	topSpace2 = new at.Tile({
		scene: scene,
		src: scene.asset.getImage(cmn.IMG_BG_STAGESTAR),
	...
	const tileBG = new at.Tile({
		scene: scene,
		src: scene.asset.getImage(cmn.IMG_MAP),
		tileWidth: cmn.SCREEN_WIDTH ,
		tileHeight: 1500,
		//
		// マップデータ
...
function createBalloonSprite( scene, balloonSpriteID ){
	...
	const blSprite = new g.Sprite({
		scene: scene,
		src: scene.asset.getImage(cmn.IMG_BALLOON),
		width: blW,
		height: blH,
		srcX: blSrcX,
		srcY: blSrcY,
...
function createCloudSprite( scene , root) {
	...
	for( i = 0; i < 3; i++){
		const bgEff = new g.Sprite({
			scene: scene,
			src: scene.asset.getImage(cmn.IMG_BG_ANIM),
			width: 211,
			height: 211,
...
function createAnimationSprite( scene, animSpriteID, posx, posy, animEndCall){
	...
	const animSprite = new g.FrameSprite({
		scene: scene,
		src: scene.asset.getImage(imageName),
		width: spW,
		height: spH,
